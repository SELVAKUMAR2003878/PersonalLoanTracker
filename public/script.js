...
async function Savefunction(name , loanAmount) {
    ...
    try {
        const response = await fetch('https://personalloantracker.onrender.com/api/loans', {
            method: 'POST',
...
async function Erase(name) {
    console.log(name);
    try {
        const response = await fetch(`https://personalloantracker.onrender.com/api/loans${name}`, {
            method: 'DELETE'
        });
        if (response.ok) {
